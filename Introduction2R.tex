...
R può essere utilizzato autonomamente tuttavia è consigliato l'utilizzo attraverso R-studio. Entrambi vanno installati separatamente e la procedura varia a seconda del proprio sistema operativo (Windows, MacOS o Linux). Riportiamo le istruzioni solo per Windows e MacOS Linux (Ubuntu)\footnote{linux}.

\footnote{linux}: Ovviamente R è disponibile per tutte le principali distribuzioni di Linux. Riportiamo qui le istruzioni per Ubuntu che sono anche valide per tutte le distribuzioni derivate.
...
\tightlist
\item
  \href{https://cran.r-project.org/}{Installare R su Ubuntu - CRAN}
\end{itemize}
